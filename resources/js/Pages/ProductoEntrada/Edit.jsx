...
export default function EditProductoEntrada({auth,productos,productoEntrada,flash}){
    
    const { data, setData, put, processing, errors } = useForm({
        cantidad: productoEntrada.cantidad,
        descripcion:productoEntrada.descripcion,
        costoUnitario: productoEntrada.costoUnitario,
        productoId: productoEntrada.producto_id,
    });

    const submit = (e) => {
        e.preventDefault();
        put(route("productoEntrada.update",{ id: productoEntrada.id }));
    };
  ...
  return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">Editar Entrada de Producto</h2>}
        >
            <div>
                <ToastContainer />
            </div>

...
export default function CategoriaIndex({ auth, categorias,mensaje }) {
    ...
    const pageCount = Math.ceil(categorias.length / clientesPerPage);

    const changePage = ({ selected }) => {
        setPageNumero(selected);
    };

    const displayedCategorias = categorias.slice(pagesVisited, pagesVisited + clientesPerPage);
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 leading-tight">LISTA DE CATEGORIAS</h2>}
        >
            <div>
                <ToastContainer />
            </div>
                    ...
                    <div className="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <div className="p-6 bg-white border-b border-gray-200">
                            <div className="flex justify-end">
                                ...
                                <Link className='px-3 py-1 text-white bg-blue-500 rounded-md focus:outline-none' href={route("categoria.create")}>
                            ...
                            <div className="overflow-x-auto mt-11">
                                {categorias && categorias.length > 0 ? (
                                                ...
                                                {displayedCategorias.map((categoria,index) => (
                                                    <tr key={categoria.id}>
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <span class={`rounded ${categoria.estado?'bg-green-400':'bg-red-400'} py-1 px-3 text-xs font-bold`}>{categoria.estado?'HABILITADO':'DESHABILITADO'}</span>
                                                        </td>
                                                        <td className="px-6 py-4 whitespace-nowrap flex ">
                                                            <Link href={`/categoria/edit/${categoria.id}`}data-tooltip-id="editCategoria" data-tooltip-content="EDITAR" className="text-indigo-600 hover:text-indigo-900 mr-2">
                                                                <Edit2 size={17} />
                                                            </Link>
                                    ...
                                    <div style={{ textAlign: 'center' }}>No hay categorias</div>
                                )}
                            </div>
                        </div>
                    </div>
